        ...
        Route(
            stop_sequence=[
                ...
                Stop(
                    ...
                    stopover_time=1.0,
                    earliest_time_of_service=768,
                    latest_time_of_service=780,
                ),
                Stop(
...
def intermediate_representation() -> Tuple[IntermediateRepresentation, IntermediateRepresentation]:
    test_routes = routes()
    ...
    return IntermediateRepresentation(vertices=vertices, arcs=arcs, routes=two_vehicles), \
           IntermediateRepresentation(vertices=vertices, arcs=arcs, routes=three_vehicles)


def build_decision_variables() -> DecisionVariables:
    test_intermediate_representation = intermediate_representation()
    ir_rep_four = test_intermediate_representation[1]

    ir_rep_four.get_vertex("S2").constructible_charger = {c2}
    ...
    decision_variables_three_standalone = DecisionVariables(
        intermediate_rep_four_simplified, {VertexID("C1"): c, VertexID("S2"): c2}, {(VertexID("D1"), VertexID("D2")): dc},
        max_speed, max_consumption, soc_init, soc_max, soc_min, energy_prices, consumption_cost
    )

    return decision_variables_three_standalone


def multi_conflict():
    decision_variables = build_decision_variables()
